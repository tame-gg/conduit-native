...
Current version: **0.9.2**. Native plugin API version: **1**.
...
To compile a plugin, `./scripts/api-jar.ps1` builds `build/conduit-api-0.9.2.jar` (the version is
`Conduit.VERSION`) and its `-sources.jar`: the `gg.tame.conduit.api` classes and nothing else. Then:

```powershell
javac --release 21 -cp build/conduit-api-0.9.2.jar -d classes src/com/example/ExamplePlugin.java
jar --create --file plugins/example.jar conduit-plugin.yml -C classes .
```

With Gradle: `compileOnly(files("path/to/conduit-api-0.9.2.jar"))`. The proxy provides the API at run time,
