 ...
 *
 * <p>There is nothing to inject. Conduit does not run a Netty pipeline for proxy I/O — a playing
 * session is relayed on Conduit's own NIO selector and worker threads — so Via is never spliced
 * into anything. What Via still needs from an injector is the pair of handler names it addresses a
 * connection by when a translation emits an extra packet of its own, and a view of which protocol
 * versions the platform is willing to be. Conduit answers with the names {@link ConduitViaSession} puts on its channel,
...
public final class ConduitViaInjector implements ViaInjector {
  ...
  public JsonObject getDump() {
    ...
    dump.addProperty("io", "NIO selector relay while playing; blocking sockets before Play");
