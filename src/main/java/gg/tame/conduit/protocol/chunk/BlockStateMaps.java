...
/**
 * Block-state ID tables for every protocol pair Conduit translates.
 *
 * <p>Tables are generated by {@code tools/gen_mappings.py} (393↔765) and
 * {@code tools/gen_mappings_pair.py} (393↔404, 404↔477) from the block-state
 ...
 * 8582 of 1.13's block states round-trip 393 -> 765 -> 393 exactly, and all 8599
 * of 1.13.2's round-trip 404 -> 477 -> 404 exactly.
 *
 * <p>Every pair needs its own table. Numeric ids are not stable even across a
 * point release: only 1126 of 1.13's 8582 states keep their meaning in 1.13.2
 * (which inserted {@code tnt[unstable]}), and only 748 of 1.13.2's 8599 keep it
 * in 1.14 (which expanded the note-block instruments). Passing ids through
 * unchanged is wrong for ~87% and ~91% of the table respectively.
 *
...
public final class BlockStateMaps {
  ...
  private static final Map<Long, int[]> TABLES = loadPairs();
  ...
  /**
   * Translates one block state across a protocol pair, by name and property set.
   *
   * <p>Identity when both sides speak the same protocol. Air when the pair has no
   * generated table or the state has no counterpart — never the raw input id,
   * which would be a different block on the far side.
   */
  public static int translate(int fromProtocol, int toProtocol, int state) {
    if (fromProtocol == toProtocol) return state;
    int[] table = TABLES.get(key(fromProtocol, toProtocol));
    if (table == null || state < 0 || state >= table.length) return AIR;
    int mapped = table[state];
    return mapped < 0 ? AIR : mapped;
  }

  /** True when a generated table backs this pair, so callers can refuse rather than guess. */
  public static boolean supports(int fromProtocol, int toProtocol) {
    return fromProtocol == toProtocol || TABLES.containsKey(key(fromProtocol, toProtocol));
  }
  ...
  private static Map<Long, int[]> loadPairs() {
    int[][] pairs = { { 393, 404 }, { 404, 477 }, { 393, 765 } };
    Map<Long, int[]> tables = new HashMap<>();
    for (int[] pair : pairs) {
      tables.put(key(pair[0], pair[1]), load("blockstates_" + pair[0] + "_to_" + pair[1] + ".bin"));
      tables.put(key(pair[1], pair[0]), load("blockstates_" + pair[1] + "_to_" + pair[0] + ".bin"));
    }
