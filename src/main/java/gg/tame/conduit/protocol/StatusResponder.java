...
public final class StatusResponder {
  ...
  public static byte[] response(ProtocolDefinition protocol, byte[] request, ServerListPingEvent ping) throws IOException {
    return answer(protocol, request, json(ping.description(), ping.versionName(), ping.versionProtocol(),
        ping.maxPlayers(), ping.onlinePlayers(), ping.samplePlayers(), ping.favicon()));
  }
  ...
  private static String json(Text description, String versionName, int versionProtocol, int max, int online,
                             List<ServerListPingEvent.SamplePlayer> sample, Optional<String> favicon) {
    ...
    json.append(",\"protocol\":").append(versionProtocol).append("},\"players\":{\"max\":").append(max)
