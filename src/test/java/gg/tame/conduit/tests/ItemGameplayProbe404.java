...
/**
 * The mirror of {@link ItemGameplayProbe}: a scripted protocol-404 client
 ...
 *
 * <pre>
 *   java -cp out gg.tame.conduit.tests.ItemGameplayProbe404 127.0.0.1 25561 Prober132
 * </pre>
 */
public final class ItemGameplayProbe404 {
  private static final ProtocolDefinition V404 = ProtocolDefinition.forVersion(404);
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    String name = arguments.length > 2 ? arguments[2] : "Prober132";
  ...
  private static void send(java.io.OutputStream out, PacketKind kind, byte[] body) throws Exception {
    MinecraftFrames.write(out,
        PlayPackets.withId(V404.id(ConnectionState.PLAY, PacketDirection.CLIENT_TO_SERVER, kind), body));
  }
  ...
  private static byte[] click(int window, int slot, int button, int action, SemanticItem clicked)
    ...
    ItemCodec.write(404, out, clicked);
    return buffer.toByteArray();
  }
  ...
  private static boolean awaitPlayLogin(java.io.InputStream in, java.io.OutputStream out) throws Exception {
    for (int attempt = 0; attempt < 4096; attempt++) {
      ...
      if (V404.is(ConnectionState.PLAY, PacketDirection.SERVER_TO_CLIENT, id, PacketKind.PLAY_LOGIN)) return true;
      if (V404.is(ConnectionState.PLAY, PacketDirection.SERVER_TO_CLIENT, id, PacketKind.PLAY_DISCONNECT)) {
  ...
  private static double[] awaitPosition(java.io.InputStream in, java.io.OutputStream out, long millis)
    ...
    while (System.currentTimeMillis() < deadline) {
      byte[] packet = MinecraftFrames.read(in, MAX_FRAME);
      int id = PlayPackets.packetId(packet);
      if (V404.is(ConnectionState.PLAY, PacketDirection.SERVER_TO_CLIENT, id, PacketKind.PLAY_PLAYER_POSITION)) {
        double[] where;
  ...
  private static boolean answer(java.io.InputStream in, java.io.OutputStream out, byte[] packet, int id)
      throws Exception {
    if (V404.is(ConnectionState.PLAY, PacketDirection.SERVER_TO_CLIENT, id, PacketKind.PLAY_KEEP_ALIVE)) {
      ByteArrayOutputStream buffer = new ByteArrayOutputStream();
      DataOutputStream reply = new DataOutputStream(buffer);
      MinecraftOutput.varInt(reply,
          V404.id(ConnectionState.PLAY, PacketDirection.CLIENT_TO_SERVER, PacketKind.PLAY_KEEP_ALIVE));
    ...
    if (V404.is(ConnectionState.PLAY, PacketDirection.SERVER_TO_CLIENT, id, PacketKind.PLAY_PLAYER_POSITION)) {
      try (DataInputStream body = new DataInputStream(new ByteArrayInputStream(PlayPackets.body(packet)))) {
        ...
        ByteArrayOutputStream buffer = new ByteArrayOutputStream();
        DataOutputStream reply = new DataOutputStream(buffer);
        MinecraftOutput.varInt(reply, V404.id(ConnectionState.PLAY,
    ...
    if (V404.is(ConnectionState.PLAY, PacketDirection.SERVER_TO_CLIENT, id,
  ...
  private static Map<String, Integer> drain(java.io.InputStream in, java.io.OutputStream out, long millis)
    ...
    while (System.currentTimeMillis() < deadline) {
      ...
      if (V404.is(ConnectionState.PLAY, PacketDirection.SERVER_TO_CLIENT, id, PacketKind.PLAY_OPEN_WINDOW)) {
        try (DataInputStream body = new DataInputStream(new ByteArrayInputStream(PlayPackets.body(packet)))) {
          ...
          MinecraftFrames.write(out, PlayPackets.withId(
              V404.id(ConnectionState.PLAY, PacketDirection.CLIENT_TO_SERVER, PacketKind.PLAY_CLICK_WINDOW),
              click(window, 0, 0, 2, SemanticItem.EMPTY)));
          MinecraftFrames.write(out, PlayPackets.withId(
              V404.id(ConnectionState.PLAY, PacketDirection.CLIENT_TO_SERVER, PacketKind.PLAY_CLOSE_WINDOW),
  ...
  private static String describe(int id) {
    for (PacketKind kind : PacketKind.values()) {
      if (V404.is(ConnectionState.PLAY, PacketDirection.SERVER_TO_CLIENT, id, kind)) return kind.name();
    }
  ...
  private static byte[] handshake(String host, int port) throws Exception {
    ...
    MinecraftOutput.varInt(out, 404);
