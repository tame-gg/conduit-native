...
| 1.21.8 (772) | 1.13 → `/server` 1.20.4 | FAILED (Conduit) | run `20260917-003208`: the 1.20.4 server disconnects the client over Client Information ("found 1 bytes extra whilst reading packet 0") replayed through a Via translator still in LOGIN, and Conduit aborts the switch with "backend finished configuration before known packs". Any switch where both the client and the target backend have a Configuration phase and Via translates between them goes through this relay; none has been verified. Open |
...
| 26.2 (776) | 1.13 → `/server` 1.20.4 | FAILED (Conduit) | same run, same two faults as 1.21.8 above; the failed switch also ends the session, because the client had already entered Configuration |
