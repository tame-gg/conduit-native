...
| When a command's `hasPermission` returns false, the player is told "You do not have permission"; Velocity would forward the command to the backend instead. LuckPerms' `/lpv` alias is such a command: it lets only the console run it, so a player's `//lpv` is refused here where Velocity would send it to the backend. | Partial | LuckPerms |
