...
| Books, dialogs, signed-message chat, `getModInfo`, `getIdentifiedKey`; and titles, the action bar, boss bars, the header, sounds and resource packs on the console as an audience | Unsupported: every one throws `UnsupportedOperationException` naming the API, including Adventure methods that are silent no-ops by default | VCT (`openBook`) |
