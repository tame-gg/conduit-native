...
**LPM** is `LoginPluginMessageTests`: plugins ask a scripted 1.20.4 client during its login and answer
a scripted 1.20.4 backend's login queries, with and without modern forwarding, natively and through a
compiled plugin; a 1.8 client cannot be asked.
...
another sends a pack from `PlayerConfigurationEvent`; a 1.8 client raises none of them. **SRT** is `ServerResourcePackTests`: a compiled plugin denies one server pack, replaces another and keeps a removal from the client, for scripted 1.20.4 (Configuration and Play) and 1.12.2 clients and servers, checked byte for byte on both sides, and 1.8's hash answers on the bookkeeping.
...
the proxy to provide. `scripts/build-jar.ps1` merges all of it into `conduit-<version>.jar`, so a
folder holding nothing but that jar loads Velocity plugins, offline, with nothing to fetch:
- velocity-api 3.4.0
- Adventure 4.26.1, including MiniMessage, the ANSI serializer and `adventure-text-logger-slf4j`
- Velocity's fork of Brigadier (the one velocity-api's POM names), Guava 33.3.1, Gson, snakeyaml
  1.33, Caffeine 3.1.8 and toml4j 0.7.2
...
from a plugin). Velocity's proxy internals (Netty's pipeline, log4j, its relocated libraries) are not
provided, and neither are libraries a plugin asks for at run time: those are the plugin's own.
...
| `getConfiguration()`: `getCompressionThreshold` is `listener.compression-threshold` (-1 when off) and `getCompressionLevel` is 4, the level Conduit deflates at; `getConnectTimeout` is 3000 and `getReadTimeout` 4000, the bounds on dialling a backend and on its login (a playing connection has no read timeout). Features Conduit does not have answer as Velocity does with them turned off: `isQueryEnabled`, `shouldQueryShowPlugins`, `isAnnounceForge` and `shouldPreventClientProxyConnections` are false, `getQueryPort` 0, `getQueryMap` "Conduit", every rate limit and kick-after count 0, and `isForwardCommandsIfRateLimited` true | Supported | VCT (`offconfig:`) |
...
| Books, dialogs and signed-message chat to a player | Unsupported: each throws `UnsupportedOperationException` naming the API, including Adventure methods that are silent no-ops by default | VCT (`openBook`) |
| `getModInfo` is empty, since Conduit does not read a modded client's mod list; `getIdentifiedKey` is null (also on a `LoginPhaseConnection`), since Conduit does not keep the client's chat key. Titles, the action bar, boss bars, the header, sounds, books, dialogs and resource packs sent to the console do nothing, as Adventure specifies for an audience that cannot show them | Supported | VCT (`offconfig:`) |
...
| The function a plugin sets answers for that player everywhere: `Player.hasPermission` / `getPermissionValue` for every Velocity plugin, and Conduit's own checks (its commands, native plugins' command permissions). Conduit hears a boolean, so UNDEFINED is "no" there, as `hasPermission` reads it. | Supported | VCT (`/nsecret` refused for Erin), LuckPerms (`/conduit doctor` and `/send` refused, then allowed after `/lpv user … permission set conduit.command.doctor true` / `conduit.command.send true`, refused again once unset; `/server` needs no node) |
| Conduit's maintenance mode lets a player in when their function grants `conduit.maintenance.bypass` or `conduit.admin` (TRUE; FALSE and UNDEFINED do not). A player the plugin set no function for is answered by Conduit's own provider, and Conduit's default never lets anyone through maintenance. A function that throws or takes over 10 seconds refuses the bypass. The maintenance allowlist by name works whatever the plugin says. | Supported | LLT, LuckPerms |
...
| When the plugin that set the functions is disabled, they stop answering and Conduit's permission provider is back for every player, the default included (which grants no `conduit.` node). A function set for a player whose setup was still running when its plugin was disabled is not kept. | Supported | VCT (vtest disabled, Erin back on the default), LLT (`Midway`), LuckPerms (`disable luckperms`) |
...
| `LoginEvent`: denying it shows the reason on the client's disconnect screen, and no backend is ever contacted. Not fired for a player Conduit's maintenance mode refuses (they get `PermissionsSetupEvent` and `DisconnectEvent` only), nor for a client that left during `PermissionsSetupEvent`, nor for a login a newer login of the same player displaced during it. The login waits for `PreLoginEvent` and `LoginEvent` handlers for at most 10 seconds each (`-Dconduit.velocity.waitMillis` changes it); one that has not finished by then refuses the login instead of letting the player in on the default result, so a slow ban check fails closed. | Fired | VCT, LLT |
| `PostLoginEvent`: once `LoginEvent` has allowed the login, before `PlayerChooseInitialServerEvent` and before any backend is dialled, as in Velocity. `getCurrentServer()` is empty. The player is not yet in `getAllPlayers()`/`getPlayer(...)` during it: Conduit counts a player online once a server has taken them | Fired (partial) | VCT |
...
| `PlayerChatEvent`: for every client Conduit reads chat from -- before 1.19, and 1.20.1, 1.20.4, 1.20.5 and 26.2 (not a 1.19 to 1.19.2 client, which Conduit has no table for). `denied()` works for all of them: a 1.19.3+ line is withheld and the backend is sent only the Message Acknowledgment count it carried, which keeps the server's record of what the client has seen in step. `message(...)` sends the backend the rewritten line instead of the typed one, before 1.19 and for an unsigned 1.19.3+ line (a client with chat signing off, or any player of an offline-mode proxy); a signed line cannot be changed without its signature failing, so its rewrite is logged and the line goes on as typed. A rewrite into a command, starting with `/`, is refused with a warning | Fired (partial) | VCT, `CommandForwardingTests` |
...
| `ProxyPreShutdownEvent`: once Conduit has stopped accepting players and before any player is disconnected. The shutdown waits for its handlers, for at most 10 seconds, so a handler that never finishes delays it rather than stopping it. | Fired | VET |
...
| `ServerResourcePackSendEvent`: every pack the backend offers the player, in Play or in the Configuration phase, before it reaches the client; the proxy waits for the handlers. Denied, the client never sees the pack and the backend gets a declined answer at once, as the client would have written it (the pack's id from 1.20.3, its hash on 1.8, the result alone in between); a backend that requires the pack then does what it does with a refusal. `setProvidedResourcePack`: the client is offered that pack instead, and its answers reach the backend as answers about the backend's own pack, rewritten to its id (1.20.3+) or hash (1.8); the result-only answers of 1.9 to 1.20.2 need no rewriting, as they are matched to offers in order. `ServerResourcePackRemoveEvent`: the backend removing one pack or every pack (1.20.3+, whose packs have ids), with the backend's id; denied, the client keeps its packs. Removing a pack a plugin replaced removes the replacement | Fired | SRT |
| `PlayerResourcePackStatusEvent`: every answer the client gives about a pack offered through the proxy, a plugin's or the backend's (`getPackInfo().getOrigin()` says which), with the pack's id and info; answers about a plugin's pack end at the proxy, answers about the backend's still reach it, and answers about a pack a plugin put in place of the backend's reach the backend as about its own (see `ServerResourcePackSendEvent`). Before 1.20.3 the answer is matched to an offer as described under Players, and the limit of 64 listed packs described there applies. Handlers are not waited for, and `setOverwriteKick` has nothing to overrule: Conduit never kicks over a declined pack. | Fired | RPT |
...
| `ServerLoginPluginMessageEvent`: when a backend the player is logging in to (first server, switch or fallback) sends a Login Plugin Request other than modern forwarding's `velocity:player_info`, which never raises it. `getConnection()` is the player's connection to that server (`getPreviousServer()` is the one they are on during a switch), with the channel, contents and sequence id. Waited for. `reply(bytes)` is sent to the backend as the player's understood answer, and the client never sees the request. `unknown()`, the default, leaves the query as Conduit handles it without plugins: during the first login, while the client is still logging in, it goes to the client and the client's answer goes back; on a switch the client can no longer answer, and the switch fails. | Fired | LPM |
| `PlayerAvailableCommandsEvent` (Conduit only appends its own commands to the root of a backend's command tree and never decodes the tree into nodes, because an argument node's parser and its properties are encoded differently across the releases it supports, by name before 1.19 and by a numeric id whose table shifts in later releases, and it has that table for 1.20.1 and 1.20.4 only), `PlayerModInfoEvent`, `CookieStoreEvent` and `CookieRequestEvent` (a backend's cookies pass through untouched), `ProxyQueryEvent` | **Never fired.** A listener for one is still registered, but Conduit logs a warning naming the handler that will never be called. | VCT (`ProxyQueryEvent`) |
