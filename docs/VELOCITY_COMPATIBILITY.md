...
| `ProxyServer` and `RegisteredServer` as audiences: titles, the action bar, boss bars, the header and footer, sounds and resource packs go to each player on the proxy, or on that server, when called, under what each player's client supports; a call a player cannot take throws as it would for that player | Supported | `VelocityDisplayTests` |
